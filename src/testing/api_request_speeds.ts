import { postsApi } from "../general_functions/API_access/posts.js";
import { FC } from "../general_functions/API_access/the_fetch_conductor.js";
import { getPosts } from "../general_functions/end_user.js";
import { easyDate, dateToId } from "../general_functions/id_timestamp_conversion.js"

export const tagBank: string[] = [
    "headband",
    "light-skinned_male",
    "smellerbee",
    "soles",
    "tomboy",
    "whisker_markings",
    "armpit_fetish",
    "armpit_hair",
    "armpit_tuft",
    "barazoku",
    "belt",
    "belt_buckle",
    "body_hair",
    "bottomwear",
    "breathing",
    "buckle",
    "bulge",
    "canis",
    "chair",
    "detailed_background",
    "eye_patch",
    "furniture",
    "gavel_(artist)",
    "hands_behind_head",
    "harness",
    "leather",
    "mechanical_leg",
    "mihoyo",
    "pixel_(artwork)",
    "pixel_animation",
    "prosthetic",
    "prosthetic_limb",
    "squish",
    "tail_motion",
    "tailwag",
    "thigh_squish",
    "thong",
    "throbbing",
    "throbbing_penis",
    "von_lycaon",
    "window",
    "wolf",
    "aidreamcrush",
    "bleach",
    "3_toes",
    "animal_penis",
    "big_balls",
    "biped",
    "black_balls",
    "black_fur",
    "black_penis",
    "black_pubes",
    "black_tail",
    "closed_eyes",
    "dialogue",
    "digitigrade",
    "dimitry_(itisd)",
    "dimka_(itisd)",
    "electric_fan",
    "ellipsis",
    "equine_genitalia",
    "equine_penis",
    "floating_eyebrows",
    "hyper",
    "hyper_genitalia",
    "itisd9",
    "medial_ring",
    "monster",
    "relaxing",
    "rotary_fan",
    "shadow",
    "slightly_chubby_anthro",
    "slightly_chubby_male",
    "sweaty_balls",
    "sweaty_genitalia",
    "sweaty_penis",
    "tan_background",
    "toes",
    "white_tongue",
    "4_toes",
    "5_fingers",
    "apollowolf",
    "arena",
    "big_arms",
    "big_chest",
    "black_claws",
    "black_markings",
    "blurred_background",
    "broad_shoulders",
    "canine_penis",
    "cheek_tuft",
    "crowd",
    "cum_in_clothing",
    "cum_through_clothing",
    "detailed_bulge",
    "digital_drawing_(artwork)",
    "dipstick_ears",
    "ear_markings",
    "erection_under_clothing",
    "facial_tuft",
    "fighting_ring",
    "fingers",
    "fist",
    "flexing",
    "genital_outline",
    "hands-free",
    "head_tuft",
    "jumbotron",
    "kartos",
    "multicolored_ears",
    "overhead_lighting",
    "penis_outline",
    "raised_fist",
    "raised_hand",
    "shaded",
    "spandex",
    "story",
    "story_in_description",
    "thick_neck",
    "toe_claws",
    "victory_pose",
    "wet_clothing",
    "wrestling_singlet",
    "anal",
    "anal_sex",
    "athletic",
    "athletic_male",
    "ball_gag",
    "bovid",
    "bovine",
    "cattle",
    "cum_splatter",
    "cutaway",
    "equid",
    "equine",
    "gag",
    "gagged",
    "holding_leash",
    "leash",
    "mountain",
    "outside",
    "penile_penetration",
    "penis_in_ass",
    "police",
    "police_hat",
    "prostate",
    "public",
    "public_nudity",
    "riding_horse",
    "side_view",
    "2021",
    "areola",
    "boo_(mario)",
    "eyebrows",
    "eyelashes",
    "flower_accessory",
    "frown",
    "glistening",
    "glistening_eyes",
    "grey_background",
    "huge_breasts",
    "ikakins",
    "innie_pussy",
    "mario_bros",
    "puffy_anus",
    "rear_view",
    "red_eyes",
    "red_sclera",
    "big_boobs",
    "big_tits",
    "boobs",
    "breasts_bigger_than_head",
    "breasts_bigger_than_torso",
    "dullvivid",
    "elora",
    "heavy_breasts",
    "spyro_the_dragon",
    "tits",
    "band-aid",
    "band-aid_on_face",
    "bandage",
    "bandage_on_face",
    "bra",
    "disembodied_penis",
    "interspecies",
    "midriff",
    "pink_body",
    "pink_fur",
    "pink_hair",
    "precum_drip",
    "prsmrti",
    "pussy_juice_drip",
    "sports_bra",
    "humanoid_pussy",
    "4_fingers",
    "carou_(vezerem)",
    "fusion",
    "generation_4_pokemon",
    "generation_6_pokemon",
    "hoopa",
    "hoopa_(unbound)",
    "huge_thighs",
    "hyper_butt",
    "hyper_hips",
    "hyper_thighs",
    "jewelry",
    "legendary_pokemon",
    "macro",
    "mesprit",
    "model_sheet",
    "pokemon_fusion",
    "smiling_at_viewer",
    "vezerem",
    "full_body",
    "hilda_(pokemon)",
    "mullon",
    "novelai",
    "pokemon_bw",
    "shorts",
    "solo_female",
    "amphibian",
    "amphibian_humanoid",
    "animal_humanoid",
    "big_teeth",
    "blonde_hair",
    "censored",
    "censored_nipples",
    "eyeless",
    "fangs",
    "frog_humanoid",
    "green_body",
    "green_skin",
    "imminent_rape",
    "imminent_sex",
    "in_heat",
    "mochimelted",
    "sharp_teeth",
    "tackle",
    "white_eyes",
    "bathroom",
    "chainsaw_man",
    "comic",
    "denji_(chainsaw_man)",
    "doggy_style",
    "hara_(harayutaka)",
    "power_(chainsaw_man)",
    "vagina",
    "amborah",
    "beverage",
    "bottle",
    "breast_expansion",
    "brown_hair",
    "container",
    "expansion",
    "gender_transformation",
    "mtf_transformation",
    "nipple_outline",
    "orangina",
    "sitting_on_swing",
    "species_transformation",
    "swing",
    "torn_clothing",
    "transformation",
    "adam_(rosycoyote)",
    "andromorph",
    "bradley_(rosycoyote)",
    "coyote",
    "eye_scar",
    "facial_scar",
    "group_sex",
    "intersex/male",
    "overweight",
    "overweight_male",
    "public_sex",
    "raccoon_dog",
    "rob_(rosycoyote)",
    "rosycoyote",
    "scar",
    "spitroast",
    "tanuki",
    "threesome",
    "fakemon",
    "hoopa_(confined)",
    "teapot_(body_type)",
    "age_difference",
    "alligator",
    "alligatorid",
    "aunt_mae",
    "bandana",
    "bed",
    "blue_scales",
    "butt_slap",
    "crocodilian",
    "deep_throat",
    "denim",
    "denim_bottomwear",
    "denim_clothing",
    "dreadlocks",
    "dress_shirt",
    "freckles",
    "fully_clothed",
    "green_scales",
    "grin",
    "jeans",
    "kerchief",
    "kostos_art",
    "legs_up",
    "lipstick",
    "makeup",
    "male_penetrating_female",
    "mating_press",
    "mature_female",
    "non-mammal_breasts",
    "older_female",
    "on_bed",
    "pants",
    "reptile",
    "round_glasses",
    "scales",
    "shirt",
    "slap",
    "slightly_chubby_female",
    "tan_body",
    "tan_fur",
    "younger_male",
    "alternative_fashion",
    "armwear",
    "boots",
    "breast_grab",
    "close-up",
    "combat_boots",
    "condom",
    "condom_wrapper",
    "domestic_dog",
    "exhibitionism",
    "fishnet_armwear",
    "fishnet_clothing",
    "german_shepherd",
    "hand_on_breast",
    "herding_dog",
    "mullet",
    "outside_sex",
    "pastoral_dog",
    "procyonid",
    "punk",
    "punkt.punkk",
    "raccoon",
    "raccoon_tail",
    "raised_bottomwear",
    "raised_clothing",
    "raised_skirt",
    "sexual_barrier_device",
    "shoes",
    "standing_doggystyle",
    "ailurid",
    "crossdressing",
    "facial_piercing",
    "feral_on_feral",
    "feral_penetrated",
    "feral_penetrating",
    "feral_penetrating_feral",
    "heart_eyes",
    "internal",
    "internal_anal",
    "nefitail",
    "nose_piercing",
    "nose_ring",
    "red_panda",
    "shinamin_ironclaw",
    "striped_markings",
    "striped_tail",
    "after_transformation",
    "assimilation",
    "brainwashing",
    "breast_milking",
    "clothing_lift",
    "comic_panel",
    "corruption",
    "duo_focus",
    "giahryan",
    "hood",
    "lactating",
    "milk",
    "nude_anthro",
    "scalie_humanoid",
    "snake",
    "transformation_sequence",
    "twinning",
    "vanessa_(giahryan)",
    "backsack",
    "cumshot",
    "eeveelution",
    "fin",
    "fin_piercing",
    "furgonomics",
    "furry-specific_piercing",
    "head_fin",
    "just_mae",
    "larger_feral",
    "red_body",
    "scolipede",
    "short_tail",
    "smaller_feral",
    "smothering",
    "tail_fin",
    "vaporeon",
    "2023",
    "big_tail",
    "black_eyebrows",
    "chart",
    "closed_smile",
    "confident",
    "confident_female",
    "corsac_fox",
    "dreamworks",
    "dudles",
    "guide_lines",
    "half-closed_eyes",
    "height_chart",
    "kung_fu_panda",
    "scared",
    "sketch_page",
    "true_fox",
    "white_inner_ear",
    "white_nose",
    "zhen_(kung_fu_panda)",
    "albinion",
    "cutie_mark",
    "friendship_is_magic",
    "hasbro",
    "my_little_pony",
    "mythological_equine",
    "pony",
    "tentacle",
    "tentacle_penetration",
    "tentacle_sex",
    "unicorn",
    "beastars",
    "clyde_wolf",
    "grey_tail",
    "holding_sheath",
    "legoshi_(beastars)",
    "notched_ear",
    "penis_tip",
    "sheath",
    "alolan_form",
    "alolan_ninetales",
    "aurorasnowtales",
    "generation_7_pokemon",
    "happy",
    "happy_sex",
    "mastery_position",
    "ninetales",
    "penis_in_pussy",
    "red_penis",
    "regional_form_(pokemon)",
    "sethy_(seththetales)",
    "snow",
    "sunset",
    "tenebscuro",
    "winter",
    "condom_balloon",
    "condom_filling",
    "doll",
    "huge_ass",
    "hyper_ass",
    "inflatable",
    "latex_suit",
    "living_inflatable",
    "absurdres",
    "bikini",
    "fade_(haircut)",
    "fingering",
    "gold_bikini",
    "hair_over_one_eye",
    "heart",
    "heart-shaped_pupils",
    "micro_bikini",
    "naito_gambo",
    "octoling",
    "purple_hair",
    "shiver_(splatoon)",
    "splatoon_(series)",
    "splatoon_3",
    "straight",
    "swimsuit",
    "symbol-shaped_pupils",
    "tentacle_hair",
    "thick_eyebrows",
    "ai-generated",
    "light_smile",
    "non-web_source",
    "petite",
    "shower_(place)",
    "showering",
    "thigh_gap",
    "wet_hair",
    "black_thighhighs",
    "blank_stare",
    "outdoors",
    "thighhighs",
    "arisu_kazumi",
    "bar_censor",
    "black_pants",
    "blue_shirt",
    "braid",
    "cleft_of_venus",
    "clothes_pull",
    "crown_braid",
    "forest",
    "hair_ornament",
    "hairclip",
    "nature",
    "on_one_knee",
    "pants_pull",
    "panty_pull",
    "pointy_ears",
    "princess_zelda",
    "the_legend_of_zelda",
    "the_legend_of_zelda:_breath_of_the_wild",
    "white_panties",
    ":d",
    "artist_name",
    "beanie",
    "black_socks",
    "cameltoe",
    "cloud",
    "collarbone",
    "commentary_request",
    "dawn_(pokemon)",
    "day",
    "grass",
    "hands_on_own_knees",
    "hat",
    "looking_down",
    "no_panties",
    "pink_footwear",
    "pissing",
    "pokemon_dppt",
    "red_scarf",
    "scarf",
    "sidelocks",
    "sky",
    "squatting",
    "white_hat",
    "auxinaten",
    "biting_own_lip",
    "cum_on_hands",
    "earrings",
    "jubilee",
    "male_pubic_hair",
    "marvel",
    "pubic_hair",
    "sunglasses",
    "x-men",
    "x-men:_the_animated_series",
    "2girls",
    "blue_ribbon",
    "chinese_commentary",
    "chinese_text",
    "completely_nude",
    "cunnilingus",
    "gradient_hair",
    "holding",
    "holding_ribbon",
    "medium_breasts",
    "miix777",
    "multicolored_hair",
    "multiple_girls",
    "path_to_nowhere",
    "rahu_(path_to_nowhere)",
    "ribbon",
    "shalom_(path_to_nowhere)",
    "translation_request",
    "white_hair",
    "yuri",
    "arturfox",
    "femboy",
    "stockings",
    "bell",
    "bell_collar",
    "crown",
    "green_penis",
    "guardian_spirit",
    "handwear",
    "headgear",
    "nero_the_nimbat",
    "novabeast",
    "ori_(series)",
    "purple_penis",
    "tyln7000",
    "warden006",
    "wardy",
    "arm_above_head",
    "askart",
    "blue_highlights",
    "dress",
    "dress_only",
    "highlights_(coloring)",
    "laundry",
    "laundry_basket",
    "muffin_(askart)",
    "raised_arm",
    "red_dress",
    "cum_on_face",
    "cum_on_penis",
    "huge_filesize",
    "linnie_(taidumtime)",
    "purple_body",
    "purple_fur",
    "sofa",
    "taidum",
    "taidumtime",
    "ambiguous_species",
    "ash_(chaboifap)",
    "ball_lick",
    "crop_top",
    "forced",
    "forced_oral",
    "homophobic_slur",
    "leash_pull",
    "licking",
    "lock",
    "male_rape",
    "male_raped",
    "male_rimming_male",
    "musk",
    "poorsal",
    "profanity",
    "pubic_mound",
    "rape",
    "school_uniform",
    "slur",
    "tenting",
    "uniform",
    "aged_up",
    "bootlegjones",
    "breast_play",
    "brother_(lore)",
    "brother_and_sister_(lore)",
    "caleb_(bootlegjones)",
    "caroline_(bootlegjones)",
    "incest_(lore)",
    "penis_lick",
    "poodle",
    "sibling_(lore)",
    "sister_(lore)",
    "smoking",
    "titjob",
    "bandai_namco",
    "black_tongue",
    "digimon",
    "digimon_(species)",
    "flirting",
    "graduation_cap",
    "legband",
    "renamon",
    "theo_(warden006)",
    "yellow_body",
    "yellow_fur",
    "accessory",
    "arm_tuft",
    "barely_visible_balls",
    "barely_visible_genitalia",
    "belly_nipples",
    "blue_markings",
    "brown_areola",
    "brown_nipples",
    "curved_tail",
    "elbow_tuft",
    "featureless_crotch",
    "flustered",
    "gynomorph/female",
    "hair_accessory",
    "happy_trail",
    "head_lick",
    "mu_suwi_(polyalloys)",
    "mxx_(mxxvous)",
    "overweight_anthro",
    "overweight_female",
    "pink_markings",
    "pink_tail",
    "polyalloys",
    "ponytail",
    "rat",
    "round_ears",
    "signature",
    "sweatdrop_(iconography)",
    "yellow_markings",
    "black_nose",
    "canid_taur",
    "canine_taur",
    "deidra_stormpaw",
    "e-01",
    "gynomorph_penetrating",
    "intersex_penetrating",
    "mammal_taur",
    "mask",
    "multicolored_body",
    "taur",
    "wolf_taur",
    "arm_tattoo",
    "arm_warmers",
    "armor",
    "black_eyes",
    "black_hand",
    "black_hands",
    "breast_tattoo",
    "breastplate",
    "c.a.m.",
    "cervine",
    "chest_fur",
    "chest_hair",
    "chest_tattoo",
    "chest_tuft",
    "cybernetic_arm",
    "cybernetic_hand",
    "cybernetic_limb",
    "cybernetics",
    "fluffy_ears",
    "fluffy_hair",
    "fluffy_neck_fur",
    "glowing",
    "glowing_body",
    "glowing_eyes",
    "grey_hair",
    "hybrid",
    "jackal",
    "kitsoonie",
    "kitsoonie_(character)",
    "knee_highs",
    "leg_tattoo",
    "machine",
    "metal",
    "metallic_body",
    "military",
    "on_one_leg",
    "prosthetic_arm",
    "prosthetic_hand",
    "protogen",
    "protogen_armor",
    "protogen_face",
    "protogen_visor",
    "protogenized",
    "robotic",
    "robotic_arm",
    "robotic_hand",
    "robotic_limb",
    "screen",
    "screen_face",
    "soldier",
    "special_forces",
    "standing_sex",
    "tattoo",
    "warrior",
    "yuri_chacal",
    "black_horn",
    "border",
    "ears_down",
    "holding_another",
    "inside_mouth",
    "lagomorph",
    "larger_female",
    "leporid",
    "pivoted_ears",
    "purple_tongue",
    "rabbit",
    "red_scales",
    "renita_reed",
    "slurping",
    "smaller_male",
    "white_border",
    "buckteeth",
    "dominant_female",
    "long_tongue",
    "spanish_text",
    "black_bodysuit",
    "black_clothing",
    "black_latex",
    "black_skinsuit",
    "bodysuit",
    "caprine",
    "clothed_anthro",
    "clothed_male",
    "domestic_cat",
    "el_(rem_phase)",
    "felis",
    "goat",
    "latex_bodysuit",
    "latex_skinsuit",
    "lock_bulge",
    "multicolored_fur",
    "null_bulge",
    "purple_background",
    "rem_phase",
    "ren_(rem_phase)",
    "skinsuit",
    "two_tone_body",
    "two_tone_fur",
    "vibrating",
    "2022",
    "avian",
    "beak",
    "beak_tied",
    "black_beak",
    "blindfold",
    "bondage",
    "breasts_tied",
    "chocobo",
    "feathers",
    "final_fantasy",
    "glistening_breasts",
    "multiple_scenes",
    "pink_areola",
    "pink_nipples",
    "presenting",
    "square_enix",
    "white_feathers",
    "wings",
    "wings_tied",
    "anxiboi",
    "tobi_(anxiboi)",
    "eye_contact",
    "imminent_fellatio",
    "imminent_oral",
    "looking_at_another",
    "undressing_another",
    "pink_wall",
    "red_footwear",
    "cetacean",
    "dolphin",
    "dungeon",
    "leafeon",
    "lunaris_parukia",
    "marine",
    "oceanic_dolphin",
    "one-bar_prison",
    "orca",
    "pink_pussy",
    "pokemorph",
    "toothed_whale",
    "vibrator",
    "kanroji_mitsuri",
    "annoyed",
    "bent_legs",
    "dark_skin",
    "feet_together",
    "interracial",
    "interracial_sex",
    "opal_bei_fong",
    "the_legend_of_korra",
    "black_anus",
    "brown_tail",
    "sarah_dona_(oc)",
    "shrose",
    "throbbing_clitoris",
    "cosplay",
    "cult_of_the_lamb",
    "macamacrez",
    "massive_monster_(studio)",
    "selvia",
    "chivaran",
    "lava",
    "mazrogal",
    "volcano",
    "alternate_costume",
    "curvaceous",
    "diantha_(pokemon)",
    "pokemon_xy",
    "all_fours",
    "cinderace",
    "generation_8_pokemon",
    "meowth",
    "red_fur",
    "rimming",
    "sinnaspice",
    "fatal_fury",
    "king_of_fighters",
    "kunoichi",
    "mai_shiranui",
    "ninja",
    "on_all_fours",
    "pussy_ejaculation",
    "pussy_juice",
    "sasizume_s",
    "snk",
    "ambiguous_gender",
    "butt_sniffing",
    "male/ambiguous",
    "sniffing",
    "angel",
    "black_shirt",
    "black_topwear",
    "body_writing",
    "colored",
    "fate_(ladyluckfate)",
    "feathered_wings",
    "fish",
    "grey_skin",
    "hyper_inflation",
    "jar",
    "ladyluckfate",
    "liquid_inflation",
    "malleable_body",
    "one_wing",
    "pattern_bottomwear",
    "pattern_clothing",
    "pattern_panties",
    "pattern_underwear",
    "pink_eyewear",
    "pink_glasses",
    "public_use",
    "red_bottomwear",
    "red_highlights",
    "red_skirt",
    "running_makeup",
    "shark",
    "spherification",
    "striped_bottomwear",
    "striped_clothing",
    "striped_panties",
    "striped_underwear",
    "tally_marks",
    "through_wall",
    "tip_jar",
    "twinmill_(ladyluckfate)",
    "water_inflation",
    "beach",
    "chest_spike",
    "cum_on_balls",
    "cum_on_chest",
    "cum_on_leg",
    "cum_on_self",
    "justtoast",
    "lucario",
    "penile_masturbation",
    "redventus",
    "sand",
    "seaside",
    "spikes",
    "spikes_(anatomy)",
    "swimwear",
    "water",
    "3d_(artwork)",
    "4k",
    "backlighting",
    "better_version_at_source",
    "bikini_top",
    "blender_(artwork)",
    "blender_eevee",
    "bracelet",
    "cock_ring",
    "covered_eyes",
    "elbow_gloves",
    "gold_(metal)",
    "gold_jewelry",
    "hair_in_face",
    "hand_on_hip",
    "leah_(akiradrawsstuff)",
    "leggings",
    "light",
    "lighting",
    "medium_hair",
    "messy_hair",
    "penis_accessory",
    "penis_jewelry",
    "ring",
    "turborenegade",
    "two-piece_swimsuit",
    "warm_colors",
    "warm_lighting",
    "american_opossum",
    "anthro_penetrated",
    "anthro_penetrating",
    "anthro_penetrating_anthro",
    "barbel_(anatomy)",
    "black_scarf",
    "blue_eyewear",
    "blue_hat",
    "blue_headwear",
    "blue_sunglasses",
    "bow_tie",
    "catfish",
    "dirty_talk",
    "fantasizing",
    "felino",
    "frostbite_(felino)",
    "grey_scales",
    "grey_scarf",
    "headwear",
    "male_on_bottom",
    "male_on_top",
    "marsupial",
    "mephitid",
    "mostly_nude",
    "onomatopoeia",
    "pattern_scarf",
    "purple_glans",
    "red_bow_tie",
    "red_collar",
    "red_eyewear",
    "red_headband",
    "red_sunglasses",
    "skunk",
    "striped_scarf",
    "thought_bubble",
    "white_scales",
    "ziggy_(felino)",
    "commanderthings",
    "curvy_figure",
    "eyewear_only",
    "glasses_only",
    "voluptuous",
    "wearing_glasses",
    "finger_fuck",
    "irwin",
    "iudi_(iudicium86)",
    "iudicium86",
    "magic",
    "magical_masturbation",
    "magical_stimulation",
    "pregnant",
    "pregnant_intersex",
    "pregnant_male",
    "big_breasts",
    "blush",
    "completely_nude_female",
    "curvy",
    "cute",
    "cute_face",
    "detailed",
    "dinixdream",
    "eyeshadow",
    "female_only",
    "fit",
    "fit_female",
    "focus",
    "gerudo",
    "hentai",
    "high_quality",
    "legs",
    "light-skinned_female",
    "light_skin",
    "lips",
    "looking_at_viewer",
    "mascara",
    "mature",
    "naked",
    "navel",
    "nintendo",
    "nude",
    "patreon",
    "patreon_username",
    "pool",
    "poolside",
    "posing",
    "pussy",
    "realgfai",
    "realistic",
    "riju",
    "seductive",
    "seductive_look",
    "slim",
    "stable_diffusion",
    "standing",
    "tagme",
    "tears_of_the_kingdom",
    "teenager",
    "thick_ass",
    "thick_butt",
    "young",
    "younger_female",
    "angel_girl",
    "ass_focus",
    "aureola",
    "back",
    "back_view",
    "bare_arms",
    "bare_shoulders",
    "blue_eyes",
    "clothed",
    "clothing",
    "color",
    "dark-skinned_female",
    "dryvial",
    "female_focus",
    "hazbin_hotel",
    "hi_res",
    "large_breasts",
    "nipples_visible_through_clothing",
    "sera_(hazbin_hotel)",
    "solo",
    "vivienne_medrano",
    "1girl",
    "mole_on_breast",
    "oc",
    "original_character",
    "rabbit_ears",
    "short_hair",
    "sitting",
    "smiling",
    "tank_top",
    "kurenai_yuhi",
    "naruto",
    "naruto_(classic)",
    "naruto_(series)",
    "naruto_shippuden",
    "yuuhi_kurenai",
    "1girl2boys",
    "big_penis",
    "blue_body",
    "demon",
    "groping",
    "itisspermo",
    "one_eye_closed",
    "open_mouth",
    "original",
    "original_characters",
    "penis",
    "1futa",
    "abs",
    "absurd_res",
    "ai_assisted",
    "anus",
    "anus_focus",
    "big_butt",
    "big_thighs",
    "cleavage",
    "dickgirl",
    "fit_futanari",
    "foreskin",
    "front_view",
    "futanari",
    "high_resolution",
    "highres",
    "inviting",
    "inviting_to_sex",
    "large_hips",
    "large_penis",
    "muscular",
    "muscular_ass",
    "muscular_female",
    "muscular_futanari",
    "muscular_thighs",
    "naked_female",
    "naked_futanari",
    "nude_futanari",
    "pale-skinned_female",
    "pale-skinned_futanari",
    "pale_skin",
    "perineum",
    "presenting_anus",
    "presenting_hindquarters",
    "spread_legs",
    "sweat",
    "sweating",
    "testicles",
    "thecoomjurer",
    "uncut",
    "wide_thighs",
    "3d",
    "eating_pussy",
    "fugtrup",
    "licking_pussy",
    "looking_at_partner",
    "looking_pleasured",
    "mei_(overwatch)",
    "open_legs",
    "overwatch",
    "overwatch_2",
    "tracer",
    "voluptuous_female",
    "boobi_(itisspermo)",
    "ghost",
    "one_eye_obstructed",
    "white_body",
    "angel_wings",
    "emily_(hazbin_hotel)",
    "3girls",
    "braided_ponytail",
    "bunny_ears",
    "bunny_tail",
    "demon_girl",
    "eyepatch",
    "hair_bun",
    "looking_back",
    "makima_(chainsaw_man)",
    "partially_clothed",
    "purple_eyes",
    "quanxi_(chainsaw_man)",
    "red_hair",
    "reze_(chainsaw_man)",
    "vestartist",
    "yellow_eyes",
    "konan",
    "serving_tray",
    "cloudy_(itisspermo)",
    "english",
    "grey_body",
    "guyspermo_(itisspermo)",
    "spermo_(itisspermo)",
    "bbw",
    "big_lips",
    "chubby",
    "cookies",
    "cooking",
    "curly_hair",
    "gloves",
    "kailewds",
    "plump_lips",
    "sabobonds",
    "vanilla_the_rabbit",
    "1boy",
    "androgynous",
    "crossplay",
    "finger_in_mouth",
    "fringe",
    "leather_pants",
    "necktie",
    "smile",
    "vanessa_(kof)",
    "white_shirt",
    "bare_thighs",
    "green_hair",
    "inhoso",
    "one-punch_man",
    "small_breasts",
    "tatsumaki",
    "cum",
    "cumshotcum_on_chest",
    "dick_piercing",
    "jerking_off",
    "kirk_hammett",
    "masturbation",
    "metallica",
    "musician",
    "piercings",
    "solo_male",
    "stoned",
    "stoner",
    "tattoos",
    "weed",
    "2boys",
    "animatronic",
    "ass_up",
    "bubble_ass",
    "bubble_butt",
    "bunny_boy",
    "ear",
    "ear_grab",
    "fnaf",
    "gay",
    "grabbing_ears",
    "huge_cock",
    "male_only",
    "massive_ass",
    "pronebone",
    "robot",
    "thunder_thighs",
    "toy_bonnie_(fnaf)",
    "twink",
    "yaoi",
    "yomiee",
    "blue_background",
    "femspermo_(itisspermo)",
    "precum",
    "amelia_raevert",
    "animal_ears",
    "beach_towel",
    "black_hair",
    "dat_ass",
    "ear_piercing",
    "fat_ass",
    "large_ass",
    "lying",
    "lying_on_stomach",
    "mistpirit",
    "ocean",
    "piercing",
    "png",
    "sunbathing",
    "sunglasses_on_head",
    "sweaty_ass",
    "sweaty_body",
    "umbrella",
    "ellen_joe",
    "fins",
    "hoyoverse",
    "maid_headdress",
    "monochrome",
    "shark_girl",
    "shark_tail",
    "smirk",
    "snowy_yukino",
    "belly",
    "bileshroom",
    "boyfriend_to_death",
    "chibi",
    "chubby_belly",
    "chubby_male",
    "dad_bod",
    "himbo",
    "hyena_ears",
    "male",
    "male_focus",
    "man_boobs",
    "shirtless",
    "strade",
    "strongfat",
    "you_kill_me_every_time",
    "blue_skin",
    "graveyard",
    "jane_doe_(metal_slug)",
    "metal_slug",
    "metal_slug_attack",
    "nikote",
    "pink_eyes",
    "tombstone",
    "v_sign",
    "zombie",
    "cum_leaking",
    "eating_vagina",
    "embarrassed",
    "glasses",
    "hot_spring",
    "licking_vagina",
    "looking_away",
    "lying_on_back",
    "nipples",
    "1920x1080",
    "1female",
    "1girl1futa",
    "2024",
    "animated",
    "cowgirl_position",
    "cum_on_breasts",
    "elf",
    "elf_ears",
    "fern_(sousou_no_frieren)",
    "frieren",
    "human",
    "lingerie",
    "longer_than_one_minute",
    "moaning",
    "oral",
    "oral_sex",
    "panties",
    "sloppy",
    "sloppy_blowjob",
    "smaller_futanari",
    "sousou_no_frieren",
    "tavern",
    "vaginal_penetration",
    "video",
    "yakimoon",
    "astronauts",
    "badcompzero",
    "blizzard_entertainment",
    "juno_(overwatch)",
    "licking_lips",
    "adawndraws",
    "adventure_time",
    "alternate_ass_size",
    "ass_bigger_than_head",
    "bangs",
    "bent_over",
    "bottom_heavy",
    "cartoon_network",
    "fake_animal_ears",
    "fionna_the_human_girl",
    "hood_up",
    "kneeling",
    "mob_face",
    "shiny_skin",
    "socks",
    "steam",
    "thighhigh_socks",
    "anime",
    "anime_screencap",
    "anime_style",
    "armpits",
    "bleach:_the_thousand-year_blood_war",
    "blowjob",
    "blowjob_face",
    "breasts_out",
    "dick_sucking_lips",
    "feet",
    "fellatio",
    "flirting_with_viewer",
    "girl",
    "seductive_eyes",
    "seductive_smile",
    "sex",
    "sexy",
    "shihouin_yoruichi",
    "smug",
    "solo_focus",
    "sucking_penis",
    "thick_legs",
    "1animal",
    "ahe_gao",
    "ahegao",
    "animal_genitalia",
    "erbrobruh",
    "fate/grand_order",
    "fate_(series)",
    "flared_penis",
    "horse",
    "horsecock",
    "interspecies_pregnancy",
    "okita_souji_(alter)_(fate)",
    "pregnant_sex",
    "uncensored",
    "zoophilia",
    "ass_grab",
    "carpet",
    "choker",
    "colored_nails",
    "high_heels",
    "hugging",
    "julia_(tdi)",
    "mk_(tdi)",
    "night",
    "pantyhose",
    "poster",
    "ripped_pants",
    "roa80h",
    "text",
    "total_drama_island",
    "total_drama_island_(2023)",
    "black",
    "black_skin",
    "cuntboy",
    "dark_skinned_male",
    "dirk_strider",
    "foot_job",
    "full_color",
    "homestuck",
    "intersex",
    "male_breasts",
    "manboobs",
    "muttbotics",
    "partially_clothed_male",
    "short_hair_male",
    "spiky_hair",
    "zebcrush",
    "blue_hair",
    "junker_queen",
    "tongue_out",
    "andrew_graves",
    "ashley_graves",
    "asymmetrical_docking",
    "black_choker",
    "black_sweater",
    "genderswap_(mtf)",
    "holding_hands",
    "incest",
    "kaizen_(artist)",
    "no_pants",
    "off_shoulder",
    "off_shoulder_sweater",
    "rule_63",
    "siblings",
    "sisters",
    "smile_at_viewer",
    "sweater",
    "the_coffin_of_andy_and_leyley",
    "white_background",
    "brown_eyes",
    "crishdemons",
    "fully_nude",
    "maria_mikhailovna_kujou",
    "tokidoki_bosotto_russian_de_dereru_tonari_no_alya-san",
    "1boys",
    "alien",
    "aviator_goggles",
    "aviator_sunglasses",
    "dildo",
    "goggles",
    "homestuck_troll",
    "horn",
    "horns",
    "horuss_zahhak",
    "long_ears",
    "masculine",
    "masturbating",
    "nude_male",
    "nudity",
    "sex_toy",
    "sharp_ears",
    "toy",
    "troll",
    "unusual_skin",
    "unusual_skin_color",
    "viktor",
    "arms_up",
    "bunnysuit",
    "double_v",
    "nayuta_(chainsaw_man)",
    "peace_sign",
    "ktrr34",
    "roblox",
    "robloxian",
    "rr34",
    "between_buttocks",
    "broken_furniture",
    "destruction",
    "doorframe",
    "dotodonoto",
    "indoors",
    "legwear",
    "orange_hair",
    "princess",
    "royalty",
    "valerie_ingenspalda_(dotodonoto)",
    "alien_humanoid",
    "amporussy",
    "amporussy_enjoyer",
    "black_hair_female",
    "cock",
    "colored_skin",
    "dick",
    "humanoid",
    "humanoid_alien",
    "plushie",
    "plushophilia",
    "saggy_breasts",
    "small_penis",
    "terezi_pyrope",
    "vicvi",
    "weird",
    "avatar_legends",
    "avatar_the_last_airbender",
    "earth_kingdom",
    "facepaint",
    "fan",
    "kimono",
    "kyoshi_warrior",
    "night_sky",
    "nonbender",
    "rabbitslides",
    "suki",
    "warpaint",
    "dilf",
    "fionna_and_cake",
    "hide_and_seek",
    "kidnapped",
    "older_male",
    "prismo_(adventure_time)",
    "shower",
    "simon_petrikov",
    "wet_body",
    "zoryan_kvit",
    "breast_awe",
    "looking_at_breasts",
    "skinny",
    "skinny_female",
    "sleepy",
    "surprised",
    "wet",
    "anal_penetration",
    "cheating",
    "double_penetration",
    "gangbang",
    "high_school_dxd",
    "looking_at_penis",
    "mosaic_censoring",
    "muscular_male",
    "puffy_nipples",
    "rias_gremory",
    "siahken",
    "sitting_on_person",
    "spreading_legs",
    "owen_moore",
    "spider",
    "web",
    "bestiality",
    "farmgirl",
    "missionary_position",
    "stomach_bulge",
    "bunny_(the_first_descendant)",
    "the_first_descendant",
    "viessa_(the_first_descendant)",
    "bare_breasts",
    "bare_legs",
    "breast_press",
    "cool_world",
    "dookie_arts",
    "holli_would",
    "milf",
    "open_clothes",
    "open_shirt",
    "white_skin",
    "2d",
    "backboob",
    "bottomless",
    "chubby_female",
    "eyewear",
    "gold_lipstick",
    "looking_back_at_viewer",
    "nezulet",
    "round_ass",
    "simple_background",
    "skindentation",
    "sweaty",
    "sweaty_back",
    "sweaty_breasts",
    "sweaty_butt",
    "tan_lines",
    "tan_skin",
    "tight_clothing",
    "tinted_eyewear",
    "yellow_lipstick",
    "1boy1girl",
    "airpods",
    "mario_and_luigi_(series)",
    "sphere_creature",
    "starlow",
    "suprise_sex",
    "surprise_buttsex",
    "amy_rose",
    "anno_morana",
    "annussymorussy",
    "anthro",
    "audible_creampie",
    "blaze_the_cat",
    "bodily_fluids",
    "book",
    "cum_in_pussy",
    "cum_inside",
    "duo",
    "ejaculation",
    "eulipotyphlan",
    "exposed_torso",
    "felid",
    "feline",
    "female/female",
    "footwear",
    "from_front_position",
    "full-package_futanari",
    "furry",
    "genital_fluids",
    "genitals",
    "group",
    "hedgehog",
    "huge_butt",
    "intersex/female",
    "loop",
    "loud_ejaculation",
    "mammal",
    "moan",
    "plumenjoyerse",
    "reading",
    "reading_book",
    "remake",
    "sega",
    "sidewaveformat",
    "sonic_(series)",
    "sonic_the_hedgehog_(series)",
    "sound",
    "sound_edit",
    "testicles_clench",
    "third-party_edit",
    "trio",
    "voice_acted",
    "aimless",
    "clothed_female",
    "cuckold",
    "cuckquean",
    "curly_hair_female",
    "kanaya_maryam",
    "long_hair_female",
    "netorare",
    "ntr",
    "roxy_lalonde",
    "short_hair_female",
    "sweatdrop",
    "vampire",
    "!",
    ".",
    "...",
    "2020s",
    "2d_(artwork)",
    "2d_artwork",
    "amaterasu",
    "anthro_female",
    "anthro_focus",
    "anthro_only",
    "anthrofied",
    "background",
    "big_hips",
    "canid",
    "canid_humanoid",
    "canine",
    "canine_humanoid",
    "closed_mouth",
    "clothes",
    "clouds",
    "cropped",
    "cropped_legs",
    "curvy_body",
    "curvy_female",
    "curvy_hips",
    "curvy_thighs",
    "daytime",
    "digital_media_(artwork)",
    "ears_up",
    "eyes",
    "eyes_open",
    "fanart",
    "female_anthro",
    "fingernails",
    "fluffy",
    "fluffy_tail",
    "fox",
    "fox_ears",
    "fox_girl",
    "fox_humanoid",
    "fox_tail",
    "fur",
    "furry_breasts",
    "furry_ears",
    "furry_female",
    "furry_only",
    "furry_tail",
    "half-dressed",
    "half_naked",
    "hourglass_figure",
    "japanese_mythology",
    "kitsune",
    "large_boobs",
    "lifeguard",
    "light_body",
    "long_tail",
    "mammal_humanoid",
    "marking",
    "markings",
    "mleonheart",
    "mouth",
    "mouth_closed",
    "multicolored_tail",
    "mythology",
    "nail",
    "nails",
    "neck",
    "neck_tuft",
    "nipple",
    "nipple_bulge",
    "no_bra",
    "no_bra_under_clothes",
    "no_humans",
    "non-human",
    "nsfw",
    "okami",
    "okami_(capcom)",
    "partially_clothed_female",
    "partially_nude",
    "revealing_clothes",
    "revealing_clothing",
    "revealing_outfit",
    "revealing_swimsuit",
    "sexually_suggestive",
    "sitting_down",
    "sitting_on_chair",
    "slim_girl",
    "snout",
    "speech",
    "speech_bubble",
    "suggestive",
    "suggestive_dialogue",
    "suggestive_look",
    "sunlight",
    "tail",
    "talking",
    "talking_to_viewer",
    "teeth",
    "teeth_showing",
    "teeth_visible",
    "text_box",
    "text_bubble",
    "text_on_clothing",
    "tight_clothes",
    "tight_fit",
    "tongue",
    "two_tone_tail",
    "video_game",
    "video_game_character",
    "video_game_franchise",
    "video_games",
    "white_fur",
    "wristwear",
    "zipper",
    "zipper_down",
    "amadeen",
    "charlie_morningstar_(hazbin_hotel)",
    "topwear",
    "cum_in_mouth",
    "cum_on_tongue",
    "defeated",
    "defeated_villainess",
    "deserved_fate",
    "deserved_rape",
    "facial",
    "lilith-soft",
    "murakami_teruaki",
    "oboro_(taimanin_asagi)",
    "orgasm",
    "rope_between_breasts",
    "rope_bondage",
    "sex_slave",
    "taimanin_(series)",
    "taimanin_asagi",
    "villainess",
    "brown_shoes",
    "massive_penis",
    "moon",
    "unaware",
    "animepixxx",
    "ass_grab_while_doggystyle",
    "battle_armor",
    "bronze_saints",
    "claudius",
    "couple",
    "creampie",
    "cum_drip",
    "cum_dripping",
    "cum_dripping_from_ass",
    "cum_in_ass",
    "cumdrip",
    "cumming",
    "cumming_inside",
    "cumming_while_penetrating",
    "cumshot_in_ass",
    "doggy_style_position",
    "exposed_breasts",
    "female_penetrated",
    "from_behind",
    "from_behind_position",
    "grabbing",
    "grabbing_ass",
    "grabbing_from_behind",
    "grabbing_own_hair",
    "hetero",
    "hetero_sex",
    "heterosexual",
    "male/female",
    "male_penetrating",
    "masked",
    "masked_female",
    "ophiuchus_shaina",
    "pegasus_seiya",
    "penis_inside",
    "ripped",
    "ripped_clothing",
    "saint_seiya",
    "sex_from_behind",
    "shounen_jump",
    "silver_saints",
    "deepthroat",
    "fire_nation",
    "lilithcharlott2",
    "zuko",
    "aradia_megido",
    "cataclysmicorgasm",
    "fat",
    "fat_female",
    "fat_woman",
    "jackalopenut",
    "large_thighs",
    "nipple_piercing",
    "nipple_piercings",
    "stomach",
    "tummy",
    "accurate_art_style",
    "drooling",
    "kiss_mark",
    "kissing",
    "shonecklace",
    "toilet",
    "areola_bulge",
    "areolae",
    "areolae_visible_through_clothing",
    "arms_behind_back",
    "ball_bulge",
    "balls",
    "balls_under_clothes",
    "barefoot",
    "big_areola",
    "big_areolae",
    "big_nipples",
    "cleavage_cutout",
    "commission",
    "dark_hair",
    "drenched",
    "erect_nipples",
    "flaccid",
    "flaccid_penis",
    "futa_focus",
    "futa_only",
    "futanari_transformation",
    "gym",
    "gym_clothes",
    "gym_clothing",
    "gym_shorts",
    "gym_uniform",
    "hair",
    "hung_futanari",
    "hyper_balls",
    "hyper_testicles",
    "irelia_xan",
    "large_areolae",
    "league_of_legends",
    "league_of_legends:_wild_rift",
    "leaking",
    "leaking_precum",
    "light-skinned_futanari",
    "low_hanging_balls",
    "massive_balls",
    "massive_breasts",
    "muscular_legs",
    "noonyx",
    "penis_bulge",
    "penis_under_clothes",
    "pony_diffusion_xl",
    "pov",
    "pov_eye_contact",
    "precum_on_ground",
    "precum_string",
    "precum_through_clothing",
    "presenting_penis",
    "public_exposure",
    "public_indecency",
    "riot_games",
    "sagging_balls",
    "saggy_balls",
    "see-through",
    "see-through_bra",
    "see-through_clothing",
    "see-through_top",
    "see_through",
    "submissive_pov",
    "taker_pov",
    "wet_clothes",
    "1femboy",
    "better_than_girls",
    "black_male",
    "bukkake",
    "dark_penis",
    "eyeliner",
    "feminine",
    "feminine_male",
    "femsub",
    "gay_domination",
    "girly",
    "hung",
    "hung_femboy",
    "link",
    "link_(breath_of_the_wild)",
    "short_shorts",
    "sissy",
    "thick",
    "thick_boy",
    "xwaifuai",
    "cow_bikini",
    "cow_horns",
    "dark-skinned_male",
    "floppydisc",
    "himeno_(chainsaw_man)",
    "penetration",
    "pleasure_face",
    "thiccwithaq_(ai_style)",
    "dvkt612",
    "solo_futa",
    "link_(tears_of_the_kingdom)",
    "belly_bulge",
    "big_belly",
    "domino_(marvel)",
    "lepetitejane1111",
    "marvel_comics",
    "vore",
    "bikini_bottom",
    "butt_crack",
    "capcom",
    "chun-li",
    "mirror",
    "mirror_reflection",
    "mirror_selfie",
    "savvs",
    "savvs0",
    "selfie",
    "street_fighter",
    "street_fighter_6",
    ":p",
    "black_bikini",
    "black_necktie",
    "black_shrug",
    "frontless_outfit",
    "heart_maebari",
    "heart_pasties",
    "highleg",
    "highleg_bikini",
    "kantai_collection",
    "kiso_(kantai_collection)",
    "maebari",
    "meme_attire",
    "partially_fingerless_gloves",
    "pasties",
    "reverse_bunnysuit",
    "reverse_outfit",
    "shrug_(clothing)",
    "sirakane_(tigo5115)",
    "tenryuu_(kantai_collection)",
    "zee_(tdi)",
    "arcade",
    "motion_lines",
    "canon_couple",
    "dave_strider",
    "karkat_vantas",
    "sketch",
    "blind",
    "disembodied_hands",
    "handstand",
    "katara",
    "shinobupp",
    "spread_ass",
    "spreading",
    "toph_bei_fong",
    "water_tribe",
    "?",
    "looking_up",
    "alien_genitalia",
    "alien_penis",
    "blonde_hair_male",
    "cuntboy_sans_balls",
    "herm",
    "hermaphrodite",
    "maleherm",
    "multi_genitalia",
    "partially_colored",
    "tentabulge",
    "vaginal_sex",
]


type Chain = {
    topLink: Promise<unknown>
    links: number
}

export async function amtChainsTest(amtChains: number, amtRequests: number, requestSize: number) {
    const chains: Chain[] = []
    for (let n = 1; n <= amtChains; n++) {
        chains.push({ topLink: Promise.resolve(), links: 0 })
    }
    FC.chains = chains

    const promises = []
    for (let n = 1; n <= amtRequests; n++) {
        const randomIndex = Math.floor(Math.random()*tagBank.length)
        const randomTag = tagBank[randomIndex]
        promises.push(postsApi(randomTag, 0, requestSize))
    }
    let avgAmtTags = 0
    for (let i = 0; i < promises.length; i++) {
        avgAmtTags += (await promises[i])[0].tags.size
        if (i%10 === 0) {
            console.log(`Fetch #${i} complete...`)
        }
    }
    avgAmtTags /= promises.length
    return avgAmtTags
}







export async function batchSizeTest(batches: number, batchSize: number): Promise<number> {
    const promises = []
    for (let n = 1; n <= batches; n++) {
        const randomIndex = Math.floor(Math.random()*tagBank.length)
        const randomTag = tagBank[randomIndex]
        promises.push(postsApi(randomTag, 0, batchSize))
    }
    let avgAmtPosts = 0
    for (const promise of promises) {
        avgAmtPosts += (await promise).length
    }
    avgAmtPosts /= promises.length
    return avgAmtPosts
}












// export async function func() {
//     const posts = await getPosts(`id:<${dateToId(easyDate(7, 27, 24))}`, 100)
//     const tags = new Set()
//     const tagBankSet = new Set(tagBank)

//     for (const post of posts) {
//         for (const tag of (post.tags).difference(tagBankSet).values()) {
//             tags.add(tag)
//         }
//     }
//     for (const tag of tags.values()) {
//         console.log(`"${tag}",`)
//     }
// }